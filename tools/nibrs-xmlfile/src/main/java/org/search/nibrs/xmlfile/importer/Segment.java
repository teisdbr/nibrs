...
import org.search.nibrs.xmlfile.util.*;
...
public class Segment
{
	...
	public List<NIBRSError> setData(ReportSource reportSource, String data)
    {
		...
		if (data.length() >= 5) {
			segmentType = NibrsStringUtils.getStringBetween(5, 5, data).charAt(0);
			if (e != null) {
				e.setSegmentType(segmentType);
			}
		}
		if (e == null) {
			String sv = NibrsStringUtils.getStringBetween(1, 4, data);
	        Integer i = null;
	        try {
	        ...
	        if (e == null) {
	        	this.segmentUniqueIdentifier = NibrsStringUtils.getStringBetween(26, 37, data);
	        	this.ori = NibrsStringUtils.getStringBetween(17, 25, data);
	        	this.actionType =  data.charAt(5);
	        	this.segmentLevel = NibrsStringUtils.getStringBetween(5, 5, data).charAt(0);
	        }
		}
        return ret;
    }
